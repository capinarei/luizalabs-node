import * as Yup from 'yup'; //é um validador
...
class UserController{
  async store(req, res){
    ...
    if (!(await schema.isValid(req.body))){
      return res.status(401).json({message: 'puts dados inválidos'})
    }
  ...
  async update(req, res){
    ...
    if(email && email != user.email){
      ...
      if(userExists){
        ...
        return res.status(400).json({message: 'Email não confere'}) //não confere
